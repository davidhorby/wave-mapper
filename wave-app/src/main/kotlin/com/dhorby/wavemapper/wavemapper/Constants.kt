...
import com.dhorby.wavemapper.wavemapper.EnvSettings.MAPS_API_KEY
import com.dhorby.wavemapper.wavemapper.EnvSettings.MAPS_API_SERVER_KEY
import com.dhorby.wavemapper.wavemapper.EnvSettings.MET_OFFICE_API_KEY

object Constants {
    val mapsApiKey: String =
        if (!EnvSettings.RUN_WITH_LOCAL_KEYS) {
            AccessSecretVersion.accessSecretVersion("mapsApiKey") ?: throw Exception("Invalid Maps API key")
        } else {
            MAPS_API_KEY
        }
    val mapsApiKeyServer: String =
        if (!EnvSettings.RUN_WITH_LOCAL_KEYS) {
...
//    }
    ...
    const val MET_OFFICE_URL = "http://datapoint.metoffice.gov.uk/public/data/val/wxmarineobs/all/xml/"
}
